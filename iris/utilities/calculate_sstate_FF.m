function [p,flg] = calculate_sstate_FF(s,ini)
...
s.Lambda = s.betta^(1/s.gamma);
    ...
    function [p] = prepare(p0)
        ...
        % firms
        p.z = p.delta+p.r; 
        p.k = p.alpha*p.mc/p.z;
        p.inv = p.k*(p.g+p.delta);
        ...
        p.profit = p.y-p.z*p.k-p.w_g*p.l-p.hc*p.h;
        % labour        
    ...
    function [dif] = get_dif(p0)
        p1 = prepare(p0);
        dif = zeros(2,1);
        dif(1) = p1.B+p1.q*p1.K*(1+p1.g)-(p1.B_o+p1.B_y+p1.B_pen);
...
opt.MAXITER = 1e6; opt.TOLFUN = 1e-6; opt.TOLX = 1e-6;

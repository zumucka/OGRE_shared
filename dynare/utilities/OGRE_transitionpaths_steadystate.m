function [ys,check] = OGRE_transitionpaths_steadystate(ys,~)
...
%% VERSION
if ~isempty(B_GDP_ss)&&~isempty(hc_w)
    hPrepare = @prepare_init;
else
    hPrepare = @prepare_term;
end
...
s.Lambda = betta^(1/gamma);
    ...
    function [p] = prepare_term(p0)
        ...
        p.inv = p.k*(1-(1-delta)/(1+p.g));
        % labour
        p.hc = kappa*p.prob_h^alpha_hc;
        p.w_g = (1-alpha)*(p.mc*(alpha/p.z)^alpha)^(1/(1-alpha)) - p.hc*(1-(1-prob_f)*(1+p.g_a)/(1+p.r));
        hc_w = p.hc/p.w_g;
        t = 1+hc_w*(1-(1-prob_f)*(1+p.g_a)/(1+p.r));
        % labour
        p.hc = hc_w*p.w_g;
